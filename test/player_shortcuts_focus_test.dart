...
/// [RecordingPlayer]) and (1) sends real key events that must reach the player
  ...
  group('player shortcuts focus', () {
    testWidgets(
      ...
      (tester) async {
        ...
        // Seeks land in the typed `seeks` record, not `calls`.
        await tester.sendKeyEvent(LogicalKeyboardKey.arrowLeft);
        expect(player.seeks, hasLength(1), reason: '← → seek (∓10s)');

        await tester.sendKeyEvent(LogicalKeyboardKey.arrowRight);
        expect(player.seeks, hasLength(2), reason: '→ → seek (∓10s)');

...
void main() {
  ...
  test(
    ...
    () {
      fakeAsync((async) {
        ...
        expect(rig.player.seeks, isEmpty, reason: '20:00 is not in the intro');
  ...
  test('the outro skip stays short of the end, so it can never complete', () {
    fakeAsync((async) {
      final outro = SkipRange(start: _s(seconds: 1200), end: _s(seconds: 1290));
      final rig = _Rig(
        episodes: [
      ...
      rig.session.start();
      async.flushMicrotasks();
      rig.player.emitDuration(_s(seconds: 1280)); // file ends inside the outro
  ...
  test('the pre-roll counts down, Cancel stops completion from advancing', () {
    fakeAsync((async) {
      ...
      async.flushMicrotasks();
      expect(rig.player.opened, hasLength(1), reason: 'cancelled = stop here');
  ...
  test(
    ...
    () {
      fakeAsync((async) {
        ...
        expect(rig.watchState.marked, hasLength(1), reason: 'one attempt only');
      });
    },
  );
  ...
  test("the engine's error is shown over the frame and logged", () {
    fakeAsync((async) {
      ...
      expect(rig.session.controls.value.errorMessage, contains('/show/1.mkv'));
  ...
  test('a watched episode opens from the start; an unwatched one resumes', () {
    fakeAsync((async) {
      final rig = _Rig(
        episodes: [
  ...
  test('dispose commits the position, stops the engine, never disposes it', () {
    fakeAsync((async) {
      final rig = _Rig(episodes: [_episode(1)]);
      rig.session.start();
      async.flushMicrotasks();

...
void main() {
  group('folder reorder re-resolves default sources', () {
    ...
    Future<void> reorder(List<Directory> order) => repo.reorderFolders([
      for (final d in order) LibraryFolder(path: d.path),
    ]);
    ...
    test('a manual per-episode source pin survives a reorder (seam #5)', () async {
      await dropEp3(folderA);
      await dropEp3(folderB);
      await sync.sync([folderA.path, folderB.path]);
    ...
    test('the new order persists (survives a relaunch)', () async {
      ...
      final reopened = DriftLibraryRepository(
        db,
        skipView: SkipViewSource.fixed(order: kBuiltInSkipOrder),
      );
      final order = (await reopened.watchedFolders())
          .map((f) => f.path)
          .toList();

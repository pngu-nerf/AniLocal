...
void main() {
  ...
  test('after an advance completes the next call is a fresh advance', () async {
    final player = RecordingPlayer();
    final resolver = _Resolver(_ep2);
    final playback = PlaybackController.withPlayer(player, resolver: resolver);
    await playback.open(_ep1);
  ...
  test('dispose is terminal: later use throws instead of rebuilding', () async {
    final player = RecordingPlayer();
    final playback = PlaybackController.withPlayer(
      player,
      resolver: _Resolver(null),

...
- [ ] **The transition is INSTANT** — the window snaps to its new size in one step, with no freeze-frame and no catch-up jump afterwards. The default macOS fullscreen animation is suppressed by the custom-animation delegate in `macos/Runner/MainFlutterWindow.swift`; if the animated resize ever returns, so does the freeze-then-jump.

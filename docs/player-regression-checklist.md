...
- [ ] **An error clears on PLAYBACK, not on a position report**: two consecutive forward steps of at most `kPlaybackStepMax` (2 s). A seek on a dead stream reports its target and a stuck stream re-reports its position; neither clears the notice (it used to flicker). The error block has a dark backing so it reads over a frozen frame as well as over black.

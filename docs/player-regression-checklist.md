...
- [ ] **⚙ › Sources** lists an episode's sources (`folder › file`) for a show in several folders, with Automatic; choosing one re-opens the same episode on that file at the SAME position (even if the episode is marked watched) and pins it — the same pin the show page's "Choose source…" writes. Absent for a single-source episode.
...
- [ ] **An error clears on PLAYBACK, not on a position report**: two consecutive forward steps of at most `kPlaybackStepMax` (2 s). A seek on a dead stream reports its target and a stuck stream re-reports its position; neither clears the notice (it used to flicker). The error block sits in an OPAQUE `XpPanel(inset: true)` well — the show page's error-state panel — so it is one look over black (failed open) and over a frozen frame (stall). Never a translucent or rounded card: nothing in the instrument is, and a translucent one read as two different error screens.
- [ ] **A dead stream has THREE signals, and each is handled**: an engine error at position 0 (a failed OPEN), a `completed` far from the end (a truncated read), and a **stall** — the position frozen for `kStallTolerance` (6 s) while the engine says it is playing, with the file probed and found gone (a volume that vanishes mid-read makes mpv emit nothing at all). A stall with the file still present is buffering and is left alone. Pause is never a stall.
...
- [ ] **A failed open on an Automatic episode falls through** to the next source at the same position, with *Playing the source in <folder> instead* over the frame for a few seconds; each source tried once. A PINNED episode is never switched — it shows the error.

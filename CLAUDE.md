...
## Current stage
**Post–Stage 6.** Stages 0–6 complete (offline cache · scan/identify · manual fix-match · multiple libraries · libmpv playback · local watch state). First post-Stage-6 features built: **multi-source episodes** (schema v7) and **folder-priority drag-reorder** (no schema change — reuses Stage 5 `sortOrder`). Remaining deferred features are built one at a time — do NOT pull more than one forward. See `ROADMAP.md`.

...
## Current stage
**Post–Stage 6.** Stages 0–6 complete (offline cache · scan/identify · manual fix-match · multiple libraries · libmpv playback · local watch state). First post-Stage-6 features built: **multi-source episodes** (schema v7), **folder-priority drag-reorder** (no schema change — reuses Stage 5 `sortOrder`), and **Up Next / auto-play next episode** (within-season; no schema change). Remaining deferred features are built one at a time — do NOT pull more than one forward. See `ROADMAP.md`.
...
Anime4K shaders · OP/ED auto-skip (AniSkip) · (maybe) JP-study dual subtitles. Do NOT start these yet.

...
- Native deps build through **CocoaPods**, declared (`pubspec.yaml` → `flutter: config: enable-swift-package-manager: false`), not fallen back to: media_kit's published Darwin plugins have no Swift Package Manager manifest (support is merged upstream but unreleased — media-kit/media-kit#1399). Flutter still prints its "plugins do not support Swift Package Manager" line on every build; that is its nag at the plugin, not a misconfiguration. Lift the setting when a media_kit release ships `Package.swift`; do not try to force the SwiftPM path before then. The dated risk (CocoaPods' registry read-only 2026-12-02) is in `ROADMAP.md`.

...
- **Per-show preferences** — ✅ **BUILT** (schema v13: `show_preferences`). Per-show cover **picture-mode** (normal / blur / removed) + **hide-next-episode**, keyed by AniList id, **sacred across refresh/rescan** (seam #5). Purely display (the cached cover is never altered); modeled as an extensible value object so new per-show prefs are one field + one column. *Full detail in `CLAUDE.md`.*
